...
const props = defineProps(["img_partenaire", "flash"]);
const form = useForm({
    img_path: "",
    // admin: "",
});
...
function sendform() {
    form.post(route("lfclean_store", "img_partenaire"));
}
...
<template>
    <AppLayout title="Create partenaire">
        <template #header>
            <div class="flex justify-between items-center">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    Créer Image partenaire
                </h2>
            </div>
        </template>

        <h1 class="text-xl font-bold mb-4 text-center">
            Mettre une image sur l'accueil Partie : Partenaire
        </h1>
        ...
        <div class="grid grid-cols-3 gap-4 justify-evenly">
            <div v-for="image in img_partenaire" :key="image.id" class="">
